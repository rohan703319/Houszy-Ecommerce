...
export async function GET() {
  ...
  try {
    const res = await fetch(`${API_URL}/api/Sitemap/categories`, { cache: 'no-store' })
    if (res.ok) {
      const json = await res.json()
      categories = json?.data ?? []
    }
  } catch (error) {
    console.error('Error fetching category sitemap data:', error)
  }

  const urls = categories
    .map((c) => {
      const date = c.lastModified ? new Date(c.lastModified).toISOString() : new Date().toISOString()
      return `  <url>
