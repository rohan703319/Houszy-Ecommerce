...
export async function GET() {
  ...
  try {
    const res = await fetch(`${API_URL}/api/Sitemap/blogposts`, { cache: 'no-store' })
    if (res.ok) {
  ...
  const urls = blogPosts
    .map((bp) => {
      const date = bp.lastModified ? new Date(bp.lastModified).toISOString() : new Date().toISOString()
      return `  <url>
