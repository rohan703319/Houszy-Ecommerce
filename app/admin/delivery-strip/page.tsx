...
export default function DeliveryStripPage() {
  ...
  const handleToggle = async (item: DeliveryStripItem) => {
    try {
      const res = await apiClient.patch<any>(`/api/DeliveryStrip/${item.id}/toggle`);
      if (res.data?.success) { toast.success(res.data.message || "Updated"); fetchItems(); }
      else toast.error(res.data?.message || "Toggle failed");
    ...
    <div className="p-6 max-w-5xl mx-auto">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="bg-slate-900 border border-slate-700 rounded-2xl shadow-2xl shadow-black/50 w-full max-w-3xl max-h-[92vh] flex flex-col">
