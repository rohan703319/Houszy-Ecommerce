        ...
        {flow.map((step, index) => {
          ...
          return (
            <div key={step.label} className="flex items-center gap-2">
              {/* Step */}
              <div
                className={`px-2.5 py-1 rounded-md text-xs font-medium border transition-all ${isActive
...
// ✅ Resolve effective payment status from payments array
// Logic: agar koi bhi payment Successful hai => Successful
//        agar koi Completed hai => Completed
//        agar koi PartiallyRefunded hai => PartiallyRefunded
//        agar koi Refunded hai => Refunded
//        agar koi Failed hai (aur koi success nahi) => Failed
//        otherwise => Pending
const resolveEffectivePaymentStatus = (order: Order): string => {
  // paymentStatus field trusted source hai agar set hai
  if (order.paymentStatus && order.paymentStatus !== 'Pending') {
    return order.paymentStatus;
  }

  const payments = order.payments ?? [];
  if (payments.length === 0) return order.paymentStatus ?? 'Pending';

  const priority = ['Successful', 'Completed', 'PartiallyRefunded', 'Refunded', 'Failed', 'Pending'];
  for (const p of priority) {
    if (payments.some(pay => pay.status === p)) return p;
  }
  return 'Pending';
};
...
const isOrderPaid = (order: Order): boolean => {
  const payments = order.payments ?? [];
  const hasSuccess = payments.some(p => ['Successful', 'Completed'].includes(p.status));
  const hasPendingOrFailed = payments.some(p => ['Pending', 'Failed', 'Processing'].includes(p.status));

  // 1. If there's no pending payment amount (or if pendingPaymentAmount <= 0)
  const hasNoPendingAmount = typeof order.pendingPaymentAmount !== 'number' || order.pendingPaymentAmount <= 0;

  // 2. Retry success condition: if payments array has 2 or more attempts, one of which is Successful, and another is Pending/Failed/Processing
  const isRetrySuccess = payments.length >= 2 && hasSuccess && hasPendingOrFailed;

  if (hasNoPendingAmount || isRetrySuccess) {
    return true;
  }

  // Fallback to effective status check
  const effectiveStatus = resolveEffectivePaymentStatus(order);
  return ['Successful', 'Completed', 'PartiallyRefunded'].includes(effectiveStatus);
};
  ...
  return (
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      ...
      <ConfirmDialog
        ...
        message={`Are you sure you want to cancel order #${order.orderNumber}? ${cancelData.initiateRefund ? 'A refund will also be initiated.' : ''
